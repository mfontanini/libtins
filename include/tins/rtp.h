...
public:
  ...
  uint16_t sequence_number() const {
    return Endian::be_to_host(header_.seq_num);
  }
   ...
   */
  const csrc_ids_type &csrc_ids() const { return csrc_ids_; }
  ...
  uint16_t extension_profile() const {
    return Endian::be_to_host(ext_header_.profile);
  }
  ...
  uint16_t extension_length() const {
    return Endian::be_to_host(ext_header_.length);
  }

  /**
   * \brief Getter for the extension header data.
   */
  const extension_header_data_type &extension_data() const { return ext_data_; }
  ...
  void sequence_number(uint16_t seq_num) {
    header_.seq_num = Endian::host_to_be(seq_num);
  }
  ...
  void timestamp(uint32_t timestamp) {
    header_.timestamp = Endian::host_to_be(timestamp);
  }
  ...
  void ssrc_id(uint32_t ssrc_id) {
    header_.ssrc_id = Endian::host_to_be(ssrc_id);
  }
  ...
  void extension_profile(uint16_t profile) {
    ext_header_.profile = Endian::host_to_be(profile);
  }
   ...
   *
   * The CSRC identifier is added after the last CSRC identifier in the
   * extension header.
   *
   ...
   *
   * If there are multiple CSRC identifiers of the given value, only the first
   * one will be removed.
   *
...
private:
  ...
  struct rtp_header {
    ...
    uint16_t version : 2, padding : 1, extension : 1, csrc_count : 4,
        marker : 1, payload_type : 7;
#elif TINS_IS_LITTLE_ENDIAN
    uint16_t csrc_count : 4, extension : 1, padding : 1, version : 2,
        payload_type : 7, marker : 1;
#endif
    uint16_t seq_num;
    uint32_t timestamp;
    uint32_t ssrc_id;
  ...
  csrc_ids_type::const_iterator
  search_csrc_id_iterator(const uint32_t csrc_id) const;
  csrc_ids_type::iterator search_csrc_id_iterator(const uint32_t csrc_id);
  extension_header_data_type::const_iterator
  search_extension_data_iterator(const uint32_t data) const;
  extension_header_data_type::iterator
  search_extension_data_iterator(const uint32_t data);
  ...
  /**
   * \brief Setter for the extension header length. Hidden from the public
   * interface. \param length The new extension header length.
   */
  void extension_length(uint16_t length) {
    ext_header_.length = Endian::host_to_be(length);
  }
